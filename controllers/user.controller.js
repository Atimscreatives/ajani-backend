...
export const getUsersByVendor = catchAsync(async (req, res) => {
  ...
  const bookingsContactInfo = await Booking.find({
    $or: [
  ...
  res.status(200).json({
    message: "Booking Contact Info retrieved successfully",
    data: bookingsContactInfo,
    count: bookingsContactInfo.length,
    vendorId: targetVendorId,
  });
});
