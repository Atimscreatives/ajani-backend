...
export const adminLogin = catchAsync(async (req, res, next) => {
  ...
  if (user.role !== "admin" && user.role !== "superadmin")
    return next(new AppError(400, "You are not authorized to login"));
...
export const adminRegister = catchAsync(async (req, res) => {
  ...
  // Save invite token to user
  user.inviteToken = inviteToken;
  await user.save();

  // Send invite email in background (don't wait for it)
  sendEmailResend({
    to: email,
    subject: "Invite to Ajani",
    html: `<p>Hi ${user.firstName || "there"},</p>
    <p>You have been invited to join Ajani. Click the link below to accept the invitation:</p>
...
// Admin Invitee
export const adminInvitee = catchAsync(async (req, res) => {
  const { email } = req.body;
